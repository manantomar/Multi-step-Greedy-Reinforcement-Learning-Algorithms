...
from stable_baselines.deepq_kpi import DQN, wrap_atari_dqn, CnnPolicy
...
def main():
    ...
    if not args.log:
        logger.configure(folder='./experiments/Atari/'+str(args.env)+'/final_nkappa_cfa0.001_new/'+str(args.kappa)+'_'+str(args.run)+'_'+str(args.phi_grad_update_freq), format_strs=["csv"])
