...
def build_train(q_func, ob_space, ac_space, optimizer, sess, grad_norm_clipping=None,
                gamma=1.0, kappa=1.0, double_q=False, scope="deepq", reuse=None,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        with tf.variable_scope("step_phi_model", reuse=False):
            step_phi_model = q_func(sess, ob_space, ac_space, 1, 1, None, reuse=False)
        q_phi_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=tf.get_variable_scope().name + "/step_phi_model")
        # target q-phi network evaluation

        with tf.variable_scope("target_q_phi_func", reuse=False):
            target_phi_policy = q_func(sess, ob_space, ac_space, 1, 1, None, reuse=False)
        target_q_phi_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                                               scope=tf.get_variable_scope().name + "/target_q_phi_func")

        # compute estimate of best possible value starting from state at t + 1
        double_q_values = None
        double_obs_ph = target_policy.obs_ph

        double_q_phi_values = None
        double_obs_phi_ph = target_phi_policy.obs_ph
        if double_q:
    ...
    with tf.variable_scope("loss", reuse=reuse):
        ...
        act_policy_phi_t_ph = tf.placeholder(tf.int32, [None], name="action_policy_phi")
        act_policy_t_ph = tf.placeholder(tf.int32, [None], name="action_policy")
        ...
        # q-phi scores for actions which we know were selected in the given state.
        q_phi_t_selected = tf.reduce_sum(step_phi_model.q_values * tf.one_hot(act_t_ph, n_actions), axis=1)
        ...
        if double_q:
            ...
            q_phi_tp1_best = tf.reduce_sum(target_phi_policy.q_values * tf.one_hot(act_policy_phi_t_ph, n_actions), axis=1)
        q_tp1_best_masked = (1.0 - done_mask_ph) * q_tp1_best
        q_phi_tp1_best_masked = (1.0 - done_mask_ph) * q_phi_tp1_best

        print("building kappa reward")
        # compute kappa-reward #TODO
        q_t_policy = tf.reduce_sum(target_phi_policy.q_values * tf.one_hot(act_policy_t_ph, n_actions), axis=1) #tf.reduce_sum(step_phi_model.q_values * tf.one_hot(act_policy_t_ph, n_actions), axis=1)
        q_t_policy_masked = (1.0 - done_mask_ph) * q_t_policy 
        rew_kappa_t_ph = rew_t_ph + gamma * (1 - kappa) * q_t_policy_masked * importance_weights_ph

        # compute RHS of bellman equation
        q_t_selected_target = rew_kappa_t_ph + gamma * kappa * q_tp1_best_masked
        q_phi_t_selected_target = rew_t_ph + gamma * q_phi_tp1_best_masked

        # compute the error (potentially clipped)
        td_error = q_t_selected - tf.stop_gradient(q_t_selected_target)
        errors = tf_util.huber_loss(td_error)
        weighted_error = tf.reduce_mean(errors) #importance_weights_ph * errors)

        td_phi_error = q_phi_t_selected - tf.stop_gradient(q_phi_t_selected_target)
        errors_phi = tf_util.huber_loss(td_phi_error)
        weighted_phi_error = tf.reduce_mean(errors_phi) #importance_weights_ph * errors_phi)

        tf.summary.scalar("td_error", tf.reduce_mean(td_error))
        tf.summary.scalar("td_phi_error", tf.reduce_mean(td_phi_error))
        ...
        update_target_phi_expr = []
        for var, var_target in zip(sorted(q_phi_func_vars, key=lambda v: v.name),
                                   sorted(target_q_phi_func_vars, key=lambda v: v.name)):
            update_target_phi_expr.append(var_target.assign(var))
        update_target_phi_expr = tf.group(*update_target_phi_expr)
        ...
        gradients_phi = optimizer.compute_gradients(weighted_phi_error, var_list=q_phi_func_vars)
        if grad_norm_clipping is not None:
            for i, (grad, var) in enumerate(gradients_phi):
                if grad is not None:
                    gradients_phi[i] = (tf.clip_by_norm(grad, grad_norm_clipping), var)
    ...
    optimize_phi_expr = optimizer.apply_gradients(gradients_phi)
    ...
    train = tf_util.function(
        inputs=[
            ...
            double_obs_ph,
            step_phi_model.obs_ph,
            target_phi_policy.obs_ph,
            double_obs_phi_ph,
            done_mask_ph,
    ...
    just_loss = tf_util.function(
        ...
        outputs=[summary, q_t_selected]
    )

    train_phi = tf_util.function(
        inputs=[
            obs_phs[0],
            act_t_ph,
            act_policy_t_ph,
        ...
        outputs=[summary, td_phi_error],
        updates=[optimize_phi_expr]
    )
    update_target = tf_util.function([], [], updates=[update_target_expr, update_target_phi_expr])

    return act_f, train, update_target, train_phi, step_model, just_loss
