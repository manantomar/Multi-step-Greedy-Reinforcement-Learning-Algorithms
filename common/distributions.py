...
class CategoricalProbabilityDistribution(ProbabilityDistribution):
    ...
    def sample(self):
        ...
        uniform = tf.random_uniform(tf.shape(self.logits), dtype=self.logits.dtype, seed=0)
...
class DiagGaussianProbabilityDistribution(ProbabilityDistribution):
    ...
    def kl_tsallis(self, other, q):
        assert isinstance(other, DiagGaussianProbabilityDistribution) 
        a = q / tf.pow(1 - q, 3/2) * (tf.pow(2*np.pi*tf.pow(tf.maximum(other.std,1e-6), 2), 1-q/2) / tf.sqrt(2*np.pi*tf.pow(self.std, 2) + tf.pow(other.std, 2)/(q-1)))
        b = tf.exp(-tf.pow(self.mean - other.mean, 2) / 2*(tf.pow(self.std, 2) + tf.pow(other.std, 2)/(q-1)))
        c = tf.pow(2*np.pi*tf.pow(tf.maximum(self.std,1e-6), 2), (1-q)/2) / (tf.sqrt(q)*(1-q))
    ...
    def sample(self):
        ...
        return self.mean + self.std * tf.random_normal(tf.shape(self.mean), dtype=self.mean.dtype, seed=0)
...
class BernoulliProbabilityDistribution(ProbabilityDistribution):
    ...
    def sample(self):
        samples_from_uniform = tf.random_uniform(tf.shape(self.probabilities), seed=0)
