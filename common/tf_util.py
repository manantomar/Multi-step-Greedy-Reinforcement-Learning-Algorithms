...
def flatgradonly(grads, var_list, clip_norm=None):
    """
    ...
    if clip_norm is not None:
        grads = [tf.clip_by_norm(grad, clip_norm=clip_norm) for grad in grads]
    return tf.concat(axis=1, values=[
        tf.reshape(grad if grad is not None else tf.zeros_like(v), [64, numel(v)]) #64 is slight hacky here, needs to be the other dimension of the tensor
        for (v, grad) in zip(var_list, grads)
    ])    
