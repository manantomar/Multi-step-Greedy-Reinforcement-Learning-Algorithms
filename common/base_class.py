...
class BaseRLModel(ABC):
    ...
    def __init__(self, policy, env, verbose=0, *, requires_vec_env, policy_base, policy_kwargs=None, policy_phi=None, policy_phi_kwargs=None):
        if isinstance(policy, str) and policy_base is not None:
            self.policy = get_policy_from_name(policy_base, policy)
        else:
            self.policy = policy
        if policy_phi is not None:
            if isinstance(policy_phi, str) and policy_base is not None:
                self.policy_phi = get_policy_from_name(policy_base, policy_phi)
            else:
        ...
        self.policy_phi_kwargs = {} if policy_phi_kwargs is None else policy_phi_kwargs
...
class OffPolicyRLModel(BaseRLModel):
    ...
    def __init__(self, policy, env, replay_buffer=None, _init_setup_model=False, verbose=0, *,
                 requires_vec_env=False, policy_base=None, policy_kwargs=None, policy_phi=None, policy_phi_kwargs=None):
        super(OffPolicyRLModel, self).__init__(policy, env, verbose=verbose, requires_vec_env=requires_vec_env,
                                               policy_base=policy_base, policy_kwargs=policy_kwargs, policy_phi=policy_phi, policy_phi_kwargs=policy_phi_kwargs)
