...
from stable_baselines.trpo_kpi.utils import traj_segment_generator, add_vtarg_and_adv, flatten_lists
...
class TRPO(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, test_env=None, gamma=0.99, timesteps_per_batch=1024, max_kl=0.01, cg_iters=10, kappa=1.0,
                 ...
                 _init_setup_model=True, policy_kwargs=None, eval_freq=10, seed=0):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("phi", reuse=False):
                    self.policy_phi = self.policy(self.sess, self.observation_space, self.action_space, self.n_envs, 1,
                                             None, reuse=False, **self.policy_kwargs)
                # Network for phi old
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    vf_phi_err = tf.reduce_mean(tf.square(self.policy_phi.value_flat - ret))
                    vf_phi_old_err = tf.reduce_mean(tf.square(self.policy_phi_old.value_flat))
                    ...
                    vf_phi_var_list = [v for v in all_var_phi_list if "/pi" not in v.name and "/logstd" not in v.name and "/q" not in v.name]
                    ...
                    self.compute_vf_phi_lossandgrad = tf_util.function([observation, self.policy_phi.obs_ph, ret],
                                                                  tf_util.flatgrad(vf_phi_err, vf_phi_var_list))
                    self.compute_vf_loss = tf_util.function([observation, old_policy.obs_ph, ret], vferr)
                    self.compute_vf_phi_loss = tf_util.function([observation, self.policy_phi.obs_ph, ret], vf_phi_err)
                    ...
                    update_phi_old_expr = []
                    for var, var_target in zip(sorted(vf_phi_var_list, key=lambda v: v.name),
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=100, tb_log_name="TRPO",
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.sess.as_default():
                ...
                vf_phi_loss_buffer = deque(maxlen=80) # rolling buffer for vf phi loss
                ...
                while True:
                    ...
                    for k in range(self.g_step):
                        ...
                        iters_val = int((2 * int(iters_so_far / (1*self.vf_phi_update_interval)) + 1) * (1*self.vf_phi_update_interval) * 1 / 2)
                        print("iters_val", iters_val, int(iters_so_far / self.vf_phi_update_interval), self.vf_phi_update_interval)
                        vf_loss = []
                        ...
                        with self.timed("vf_phi"):
                            #vf_phi_loss = []
                            for _ in range(self.vf_iters): #self.vf_phi_update_interval):
                                with self.temp_seed(self.phi_seed):
                                    for (mbob, mbret) in dataset.iterbatches((seg["observations"], seg["tdlamret_phi"]),
                                                                        include_final_partial_batch=False, 
                                                                        batch_size=128,
                                                                        shuffle=True):
                                        grad = self.allmean(self.compute_vf_phi_lossandgrad(mbob, mbob, mbret, sess=self.sess))
                                        ...
                                        self.vf_phi_adam.update(grad, self.vf_stepsize)
                                        vf_phi_loss.append(self.compute_vf_phi_loss(mbob, mbob, mbret, sess=self.sess))
                    ...
                    if len(len_buffer) > 0:
                        ...
                        logger.record_tabular("VfLoss", np.mean(vf_loss_buffer))
                        logger.record_tabular("VfPhiLoss", np.mean(vf_phi_loss_buffer))
